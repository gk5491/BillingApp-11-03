          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                  ...
                  <SelectContent>
                    {unpaidInvoices.map((inv: any) => (
                      <SelectItem key={inv.id} value={inv.id}>{inv.document_number} - Rs{Number(inv.balance_due).toLocaleString()} due</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            {selectedInvoiceRecord && (
              <div className="rounded-lg border border-border bg-muted/40 p-3 text-sm space-y-1">
                ...
                <div className="flex justify-between gap-3"><span className="text-muted-foreground">Invoice Total</span><span>Rs{Number(selectedInvoiceRecord.total || 0).toLocaleString()}</span></div>
                <div className="flex justify-between gap-3"><span className="text-muted-foreground">Remaining Amount</span><span className="font-semibold text-primary">Rs{Number(selectedInvoiceRecord.balance_due || 0).toLocaleString()}</span></div>
              </div>
            )}
              ...
              <tbody>
                {filtered.map((p: any) => (
                  <tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => navigate(`/sales/payments/${p.id}`)}>
                    ...
                    <td className="px-5 py-3 text-right font-medium text-card-foreground">Rs{Number(p.amount).toLocaleString()}</td>
