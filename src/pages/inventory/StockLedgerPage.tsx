    ...
    <div>
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-muted-foreground">No stock movements yet. Create invoices or bills to see stock changes.</div>
        ) : (
              ...
              <tbody>
                {movements.map((m: any) => (
                  <tr key={m.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3 font-medium text-card-foreground">{m.items?.name || "—"}</td>
                    ...
                    <td className="px-5 py-3 text-right text-muted-foreground">₹{Number(m.cost_price).toLocaleString()}</td>
