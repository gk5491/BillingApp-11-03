...
export default function POSPage() {
  ...
  const handleCheckout = async (mode: string) => {
    ...
    try {
      ...
      toast({ title: "Sale completed", description: `₹${total.toLocaleString()} via ${mode}` });
      setCart([]);
    } catch (err: any) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {filtered.map((p: any) => (
              ...
              >
                ...
                <p className="text-lg font-bold text-primary mt-1">₹{Number(p.selling_rate).toLocaleString()}</p>
