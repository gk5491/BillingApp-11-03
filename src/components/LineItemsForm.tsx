    ...
    setLineItems(prev => {
      ...
      if (field === "item_id") {
        ...
        if (item) {
          ...
          // Auto-fill tax from item's tax_rate_id or use default
    ...
    <div className="space-y-3">
      ...
      {lineItems.map((li, i) => (
        <div key={i} className="grid grid-cols-12 gap-2 items-center">
          ...
          <div className="col-span-2 text-right">
            <p className="text-sm font-medium text-card-foreground">₹{(li.amount + li.tax_amount).toLocaleString()}</p>
            {li.tax_amount > 0 && (
              <p className="text-[10px] text-muted-foreground">Tax: ₹{li.tax_amount.toLocaleString()}</p>
            )}
          </div>
      ...
      <div className="border-t border-border pt-3 space-y-1.5 text-sm">
        ...
        {showTax && taxTotal > 0 && (
          <>
            {isInterState ? (
              <div className="flex justify-between text-muted-foreground">
                <span>IGST</span>
                <span>₹{igstTotal.toLocaleString()}</span>
              </div>
            ) : (
              <>
                <div className="flex justify-between text-muted-foreground">
        ...
        <div className="flex justify-between font-bold text-card-foreground pt-1 border-t border-border">
          ...
          <span>₹{(subtotal + taxTotal).toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
