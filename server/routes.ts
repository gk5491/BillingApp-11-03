...
async function updateStock(itemId: string, qty: number, movementType: any, refId: string, refType: string, cost: number = 0, userId?: string) {
  ...
  await db.query`INSERT INTO stock_movements (id, item_id, movement_type, quantity, cost_price, reference_id, reference_type, created_by, created_at) 
    VALUES (${movementId}, ${itemId}, ${movementType}, ${qty}, ${cost}, ${refId}, ${refType}, ${userId || null}, GETDATE())`;
}
...
router.get("/accounts", authenticate, async (req, res) => {
  try {
    const data = await db.query`SELECT * FROM accounts ORDER BY code`.then(res => res.recordset);
    res.json(data);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.get("/stock-movements", authenticate, async (req, res) => {
  try {
    // TODO: Complex fetch stockMovements
    const dataResult = await db.query`SELECT * FROM "stockMovements"`;
...
router.get("/user-roles", authenticate, async (req, res) => {
  try {
    const data = await db.query`
      SELECT ur.id, ur.user_id, ur.role, u.email, p.display_name 
      FROM user_roles ur 
...
router.put("/user-roles/:id", authenticate, async (req, res) => {
  try {
    const { role } = req.body;
...
router.get("/warehouses", authenticate, async (req, res) => {
  try {
    const data = await db.query`SELECT * FROM warehouses ORDER BY name`.then(res => res.recordset);
    res.json(data);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.post("/warehouses", authenticate, async (req, res) => {
  try {
    const id = uuidv4();
    const { name, address, city, state, is_primary } = req.body;
    await db.query`INSERT INTO warehouses (id, name, address, city, state, is_primary, created_at, updated_at) VALUES (${id}, ${name}, ${address || null}, ${city || null}, ${state || null}, ${is_primary ? 1 : 0}, GETDATE(), GETDATE())`;
...
router.post("/inventory-adjustments", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const id = uuidv4();
    await db.query`INSERT INTO inventory_adjustments (id, date, reference_number, reason, description, adjustment_type, warehouse_id, created_by, created_at, updated_at) 
      VALUES (${id}, ${adj.date}, ${adj.reference_number || null}, ${adj.reason || null}, ${adj.description || null}, ${adj.adjustment_type}, ${adj.warehouse_id || null}, ${req.user!.id}, GETDATE(), GETDATE())`;
    if (lineItems?.length > 0) {
      for (const item of lineItems) {
        const itemId = uuidv4();
        await db.query`INSERT INTO inventory_adjustment_items (id, adjustment_id, item_id, quantity, adjustment_type) 
          VALUES (${itemId}, ${id}, ${item.item_id}, ${item.quantity}, ${item.adjustment_type})`;

        await updateStock(item.item_id, item.quantity, item.adjustment_type, id, 'Inventory Adjustment');
      }
    }
...
router.post("/stock-transfers", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const id = uuidv4();
    await db.query`INSERT INTO stock_transfers (id, date, reference_number, from_warehouse_id, to_warehouse_id, notes, created_by, created_at, updated_at) 
      VALUES (${id}, ${transfer.date}, ${transfer.reference_number || null}, ${transfer.from_warehouse_id}, ${transfer.to_warehouse_id}, ${transfer.notes || null}, ${req.user!.id}, GETDATE(), GETDATE())`;
    if (lineItems?.length > 0) {
      for (const item of lineItems) {
        const itemId = uuidv4();
  ...
  try {
    ...
    if (items && Array.isArray(items)) {
      for (let i = 0; i < items.length; i++) {
        ...
        // Update stock
        await updateStock(item.item_id, item.quantity, 'in', id, 'Bill');
      }
    }
  ...
  try {
    ...
    if (items && Array.isArray(items)) {
      for (let i = 0; i < items.length; i++) {
        ...
        await updateStock(item.item_id, item.quantity, 'in', id, 'Vendor Credit');
      }
    }
  ...
  try {
    ...
    if (items && Array.isArray(items)) {
      for (const item of items) {
        ...
        await updateStock(item.item_id, item.quantity, 'out', id, 'Purchase Return');
      }
    }
...
router.post("/expenses", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    await db.query`INSERT INTO expenses (id, date, category, vendor_id, account_id, amount, tax_amount, payment_mode, description, is_recurring, recurring_frequency, created_by, created_at, updated_at) 
      VALUES (${id}, ${date}, ${category}, ${vendor_id || null}, ${account_id || null}, ${amount}, ${tax_amount || 0}, ${payment_mode || 'cash'}, ${description || null}, ${is_recurring ?? 0}, ${recurring_frequency || null}, ${req.user!.id}, GETDATE(), GETDATE())`;
...
router.get("/invoice-items", authenticate, async (req, res) => {
  try {
    const dataResult = await db.query`SELECT TOP 500 * FROM invoice_items`;
    res.json(dataResult.recordset);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
